...
class Bot {
  constructor(options) {
    ...
    this.muteUsersIrc = options.muteUsersIrc || [];
    this.muteUsersSlack = options.muteUsersSlack || [];
  ...
  sendToIRC(message) {
    ...
    if (this.muteUsersSlack.indexOf(user.name) !== -1) {
  ...
  sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      if (this.muteUsersIrc.indexOf(author) !== -1) {
