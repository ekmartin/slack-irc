...
class Bot {
  constructor(options) {
    ...
    imgur.setCredentials(options.imgurEmail, options.imgurPassword, options.imgurClientID);
  ...
  sendToIRC(message) {
    ...
    if(message.subtype == 'file_share'){
      ...
      if(matchRule(file.mimetype, "image/*")){
        ...
        download(file.url_private_download, filePath, this.token, function(){
          imgur.uploadFile(filePath).then(function(json) {
      ...
      const regex = /(.*): ?(\w+.*)/;
      const regexMatch = regex.exec(message.text);
      if (regexMatch !== null && regexMatch.length >= 1) {
        const ircUsername = regexMatch[ 1 ];

        const ircText = ircColors.bold.red(user.name + " has sent you a message on Slack:") + "\n" + regexMatch[2];
        logger.debug('Sending message to IRC', ircUsername, ircText);
        this.ircClient.say(ircUsername, ircText);
      }
    } else {
      if (!channel) {
  ...
  sendToSlack(author, channel, text) {
    ...
    } else if(channel === this.nickname) {
      logger.debug('Received a private message on IRC. Sending to Slack');

      const { dataStore } = this.slack.rtm;
      const regex = /(.*): ?(\w+.*)/;
      const regexMatch = regex.exec(text);
      if(regexMatch !== null && regexMatch.length >= 1) {
        const slackUsername = regexMatch[1];
        const slackUser = dataStore.getUserByName(slackUsername.toLowerCase());
        if(slackUser !== null && typeof slackUser != 'undefined') {
          ...
          const options = {
            ...
            as_user: true,
            attachments: [{
              fallback: author + " has sent you a message on IRC:\n" + text,
              color: "#36a64f",
              author_name: author,
              author_icon: iconUrl,
              pretext: author + " has sent you a message on IRC.",
              text: text,
              footer: 'IRC -> Slack',
              footer_icon: this.avatarUrl.replace(/\$username/g, this.nickname),
              ts: Math.floor(Date.now() / 1000)
            }]
          };

          logger.debug('Sending message to Slack', text, channel, '->', slackUser);
          this.slack.web.chat.postMessage(slackUser.id, null, options);
        }
      }
...
function download(url, dest, auth, cb) {
