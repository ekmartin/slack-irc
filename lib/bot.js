...
Bot.prototype.attachListeners = function() {
  ...
  this.ircClient.on('message', function(nick, to, text, message) {
    ...
    logger.debug('Registered event: ', message);
    var ircChannel = this.channels[0];
    var _this = this;
    this.getMembers(ircChannel).then(
      function() {
        logger.debug('fulfilled', arguments);
        this.ircClient.say(ircChannel, _this.text.sort().join(' '));
      }.bind(this),
      function() {
        logger.debug('rejected', arguments);
        this.ircClient.say(ircChannel, 'Rejected: ' + arguments.join(' '));
      },
      function() {
        logger.debug('notified', arguments);
        this.ircClient.say(ircChannel, 'Notified: ' + arguments.join(' '));
      }
    );
  }.bind(this));
...
/**
 * Get the members from a slack group
 *
 */

Bot.prototype.getMembers = function(channel) {
  var slackChannelName = this.invertedMapping[channel];
  var channelId = this.slack.getChannelByName(slackChannelName).id;
  var deferred = vow.defer();
  var _this = this;
  this.slack._apiCall('channels.info', { channel: channelId }, function(value) {
    if (value.ok) {
      value.channel.members.forEach( function(curr) {
        _this.slack._apiCall('users.info', { user: curr }, function(user) {
          if (user.ok) {
            _this.text.push(user.user.name);
            if (value.channel.members.length === _this.text.length) {
              deferred.resolve(_this.text);
            }
          } else {
