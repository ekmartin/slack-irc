  ...
  this.slack.on('message', function(message) {
    ...
    if (message.type === 'message' && !message.subtype) {
      if (!this.silent) {
        ...
        // Send an notification back to the user that sending to irc is disabled
        var channelInfo = this.getIRCChannel(message);
        if (channelInfo === null) {
          return;
        }

        var channel = channelInfo.ircChannel;
        this.sendToSlack(this.nickname, channel, 'Sending to IRC is disabled by the Admin');
      }
    }
  }.bind(this));
...
Bot.prototype.getIRCChannel = function(message) {
...
Bot.prototype.sendToIRC = function(message) {
  ...
  var ircChannel = channelInfo.ircChannel;
