...
class Bot {
  ...
  attachListeners() {
    ...
    this.ircClient.on('topic', (channel, topic, nick) => {
      this.sendToSlack(this.nickname, channel, `*${nick}* changed the topic`);
      this.setSlackTopic(channel, topic);
    });
  ...
  setSlackTopic(channel, topic) {
    const slackChannel = this.getSlackChannel(channel);
    if (!slackChannel) {
      return;
    }
    logger.debug('Sending topic to Slack', topic, channel, '->', slackChannel.name);
  ...
  sendToSlack(author, channel, text) {
    ...
    logger.debug('Sending message to Slack', message, channel, '->', slackChannel.name);
