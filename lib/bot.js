...
class Bot {
  constructor(options) {
    ...
    const rtm = new RtmClient(options.token, { useRtmConnect: true, dataStore: false });
    this.slack = { web, rtm };
    ...
    this.slack.users_list = {}
    this.slack.conversations_list = {}
  }
  ...
  attachListeners() {
    this.slack.rtm.on('open', async () => {
      ...
      this.slack.users_list = await this.slack.web.users.list();
      this.slack.conversations_list = await this.slack.web.conversations.list();
    });
  ...
  parseText(text) {
    ...
    return text
      ...
      .replace(/<#(C\w+)\|?(\w+)?>/g, (match, channelId, readable) => {
        const { name } = this.getChannelById(channelId);
  ...
  getChannelByName(name) {
      let channel = jp.query(this.slack.conversations_list, '$..channels[?(@.name=="'+name+'")]');
      if (channel.length == 1) {
        return channel[0];
      }
      return null;
  ...
  getChannelById(id) {
      let channel = jp.query(this.slack.conversations_list, '$..channels[?(@.id=="'+id+'")]');
      if (channel.length == 1) {
        return channel[0];
      }
      return null;
  ...
  getUserById(id) {
      let user = jp.query(this.slack.users_list, '$..members[?(@.id=="'+id+'")]');
      if (user.length == 1 ) {
        return user[0];
      }
      return null;
  ...
  async sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      const channel_members_list = await this.slack.web.conversations.members(slackChannel.id);
      const currentChannelUsernames = channel_members_list.members.map(member =>
        this.getUserById(member).name
      );
