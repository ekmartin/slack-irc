...
Bot.prototype.attachListeners = function() {
  ...
  this.ircClient.on('message', function(nick, to, text, message) {
    ...
    var slackChannelName = this.invertedMapping[ircChannel.toLowerCase()];
        ...
        if (value.ok) {
         value.channel.members.forEach( function(curr) {
          _this.slack._apiCall('users.info', { user: curr }, function(user) {
            if (user.ok) {
              _this.ircClient.say(ircChannel, user.user.name);
              text.push(user.user.name);
            }
          });
         });
        }
    ...
    middle().then(function() {
      _this.ircClient.say(ircChannel, text.join(' '));
    });
