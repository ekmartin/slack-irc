...
class Bot {
  ...
  sendToIRC(message) {
      ...
      if (this.isCommandMessage(text)) {
		...
		try {
			var request = https.get(message.file.permalink_public, (res) => {
				...
				res.on('end', () => {
					try {
						real_comment = rawData.match(/<meta property=\"og:title\" content=\"([^"]*)\">/i)[1];
						real_url = rawData.match(/<a class=\"file_header\b[^"]*\".*\bhref=\"([^"]*)\">/i)[1];
						logger.info(rawData); //debug
					} catch (e) {
    ...
    if (real_url == "")
			text = `${username}** failed uploading a file`;
		else
			text = `${username}** ${real_comment} ${real_url}`;
