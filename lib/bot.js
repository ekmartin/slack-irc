...
Bot.prototype.attachListeners = function() {
  ...
  this.ircClient.on('join', function (channel, nick) {
        ...
        this.ircClient.say("#Hero6", "Welcome to the Hero6 IRC Channel!");
        this.ircClient.say("#Hero6", "The channel may seem empty, but it's not.  We're currently using Slack to centralize our development communication.  My job is to facilitate your communication with our private Slack channel.  I'll also post any messages from the developers in this channel.");
    }.bind(this));
  ...
  this.ircClient.on('quit', function (nick, reason, channels, message) {
        this.sendToSlack(this.nickname, "#Hero6", nick + " has left the IRC channel");
    }.bind(this));
