...
class Bot {
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
		...
		try {
			...
			var request = https.get(message.file.permalink_public, (res) => {
				...
				res.on('end', () => {
					try {
						...
						real_url = rawData.match(/<a class=\"file_header\b[^"]*\".*\bhref=\"([^"]*)\">/im)[1];
						logger.info(rawData.match(/<meta property=\"og:title\" content=\"([^"]*)\">/im));
						logger.info(rawData.match(/<a class=\"file_header\b[^"]*\".*\bhref=\"([^"]*)\">/im)[1]);
					} catch (e) {
    ...
    if (real_url == "")
			text = `File: ${username} failed uploading a file`;
		else
			text = `File: ${username} ${real_comment} ${real_url}`;
